  ...
  >
    ...
    >
      ...
      <el-form-item label="上级分类" prop="parent_id">
        ...
        <span class="avatar-uploader-tip">不选则为顶级分类</span>
      </el-form-item>
      <!--
...
import { edit } from '@/api/sys-manage/class-list'
  ...
  methods: {
    ...
    handleSure() {
      this.$refs.dataForm.validate((v) => {
        if (!v) return
        const { id, sort, name, parent_id } = this.temp // imageList
        const sendData = {
          id,
          sort,
          name,
    ...
    getdetail() {
      ...
      if (isEdit) {
        const { id, sort, name, parent_id } = data
