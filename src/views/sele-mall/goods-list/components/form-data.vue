          ...
          <el-form ref="form" :model="form" :rules="rules" label-width="120px">
            ...
            <el-form-item label="商品数据">
              ...
              >
                ...
                <el-table-column key="key_point_price" label="销售积分">
                  <template slot-scope="{ row, $index }">
                    <el-form-item
                      label=""
                      :rules="rules._point_price"
                      :prop="`products.${$index}._point_price`"
                    >
                      <input-cleave
                        :is-decimal="0"
                        v-model="row._point_price"
                        clearable
                      ></input-cleave>
                    </el-form-item>
                  </template>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        _point_price: [{ required: true, message: '请输入', trigger: 'blur' }]
      },
      addDialog: {
        visible: false
      }
  ...
  methods: {
    ...
    handleSave() {
      this.$refs.form.validate((v) => {
        ...
        products.map((c) => {
          ...
          _point_price.push(c._point_price)
          _spec_array.push(
            specList.map((j) => {
    ...
    handleClose(refresh = false) {
      this.$tab.closeOpenPage({ name: 'SeleMallGoods' })
      if (refresh) {
        this.$bus.emit('SeleMallGoodsUpdate')
      }
    },
